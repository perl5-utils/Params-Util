...
is(refaddr(Params::Util::_SCALAR($scalar)), refaddr($scalar), '...::_SCALAR returns the same SCALAR reference');
...
is(refaddr(_SCALAR($scalar)), refaddr($scalar), '...::_SCALAR returns the same SCALAR reference');
...
is(refaddr(Params::Util::_SCALAR0($scalar)),  refaddr($scalar),  '...::_SCALAR returns the same SCALAR reference');
is(refaddr(Params::Util::_SCALAR0($scalar0)), refaddr($scalar0), '...::_SCALAR returns the same SCALAR reference');
...
is(refaddr(_SCALAR0($scalar)),  refaddr($scalar),  '...::_SCALAR returns the same SCALAR reference');
is(refaddr(_SCALAR0($scalar0)), refaddr($scalar0), '...::_SCALAR returns the same SCALAR reference');
...
is(refaddr(Params::Util::_ARRAY($array)), refaddr($array), '...::_ARRAY($array) returns the same ARRAY reference');
...
is(refaddr(_ARRAY($array)), refaddr($array), '_ARRAY($array) returns the same ARRAY reference');
...
is(refaddr(Params::Util::_ARRAY0($array)), refaddr($array), '...::_ARRAY0($array) returns the same ARRAY reference');
...
is(refaddr(_ARRAY0($array)), refaddr($array), '_ARRAY0($array) returns the same reference');
...
is(refaddr(Params::Util::_HASH($hash)),  refaddr($hash), '...::_HASH($hash) returns the same reference',);
...
is(refaddr(_HASH($hash)),  refaddr($hash), '_HASH($hash) returns the same reference',);
...
is(refaddr(Params::Util::_HASH0($hash)),  refaddr($hash), '...::_HASH0($hash) returns the same reference',);
...
is(refaddr(_HASH0($hash)),  refaddr($hash), '_HASH0($hash) returns the same reference',);
...
is(refaddr(Params::Util::_CODE($code)),     refaddr($code),     '...::_CODE(ref) returns the same reference');
is(refaddr(Params::Util::_CODE(\&testsub)), refaddr(\&testsub), '...::_CODE(\&func) returns the same reference');
...
is(refaddr(_CODE($code)),     refaddr($code),     '_CODE(ref) returns the same reference');
is(refaddr(_CODE(\&testsub)), refaddr(\&testsub), '_CODE(\&func) returns the same reference');
...
foreach my $object (@objects)
{
    ...
    is(refaddr(Params::Util::_INSTANCE($object, 'Foo')),
        refaddr($object), '...::_INSTANCE(object, class) returns the same object');
}
